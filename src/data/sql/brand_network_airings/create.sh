...
(
   ...
   UNIQUE KEY(brand_id, airing_date, daypart, tv_network, coop_brand_id)
);
DROP TRIGGER IF EXISTS bn_airing_init$now;
DELIMITER //
CREATE TRIGGER bn_airing_init$now BEFORE INSERT ON $tblname FOR EACH ROW
	BEGIN
