...
(
   ...
   UNIQUE KEY(product_id, airing_date, daypart, tv_network, coop_product_id)
);
DROP TRIGGER IF EXISTS pn_airing_init$now;
DELIMITER //
CREATE TRIGGER pn_airing_init$now BEFORE INSERT ON $tblname FOR EACH ROW
	BEGIN
