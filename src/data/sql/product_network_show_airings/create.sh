...
(
   ...
   UNIQUE KEY(product_id, airing_date, daypart, tv_network, show_title, show_genres, coop_product_id)
);
DROP TRIGGER IF EXISTS pns_airing_init$now;
DELIMITER //
CREATE TRIGGER pns_airing_init$now BEFORE INSERT ON $tblname FOR EACH ROW
	BEGIN
    ...
    IF NEW.coop_product_id = 0 THEN
		#SET NEW.coop_product_id := NULL; # For all engines, a UNIQUE index permits multiple NULL values for columns that can contain NULL.
