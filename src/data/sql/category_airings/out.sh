...
SELECT (CASE WHEN(category_id IS NULL)THEN 0 ELSE category_id END) AS category_id,
       airing_date, sum(spend) as spend, sum(airings) as airings
FROM $tbl
$subQuery
GROUP BY category_id, airing_date;"