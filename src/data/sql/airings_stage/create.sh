...
(
   ...
   airings_type INT(3) DEFAULT 1, INDEX(airings_type),
   UNIQUE KEY(content_id, airing_start_time_UTC, tv_station_id, tv_station_tz) # Duplicated from arings_master
);
DROP TRIGGER IF EXISTS airing_stage_init$now;
DELIMITER //
CREATE TRIGGER airing_stage_init$now BEFORE INSERT ON $tblname FOR EACH ROW
	BEGIN
