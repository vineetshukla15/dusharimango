drop function if exists data_export.getTZString;
delimiter //
...
SELECT content_id, title as content_title, brand, ROUND(loc_lat, 1) AS lat, ROUND(loc_long, 1) AS lng, DATE(CONVERT_TZ(start_time_first, '+00:00',data_export.getTZString(user_tz))) AS airing_date, COUNT(*) AS viewers
FROM data_export.2016_sep_sling_data_all
